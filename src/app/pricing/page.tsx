...
export default function PricingPage() {
  ...
  return (
    <div className="flex-1 flex flex-col h-full items-center justify-start overflow-y-auto custom-scrollbar px-6 py-12 bg-gray-50/50">
      {/* Header */}
      <div className="text-center max-w-2xl w-full mb-12">
        ...
        <div className="mt-8 flex justify-center items-center gap-3">
          ...
          <span className={`text-sm font-medium flex items-center gap-1.5 ${billingCycle === "yearly" ? "text-gray-900" : "text-gray-400"}`}>
            Yearly
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-5xl items-stretch">
        {plans.map((plan) => {
          ...
          return (
            <div
              key={plan.name}
              className={`bg-white border rounded-3xl p-8 flex flex-col transition-all relative ${
                plan.primary
                  ? "border-purple-500 shadow-xl ring-2 ring-purple-100 -translate-y-2 md:-translate-y-4"
            ...
            >
              ...
              <div className="mb-6">
                ...
                <p className="text-xs text-gray-400 mt-2 leading-relaxed h-10">{plan.description}</p>
              </div>
              ...
              {/* Action Button */}
              {isFree ? (
                <div className="w-full text-center py-3 px-4 rounded-xl text-sm font-semibold bg-gray-50 text-gray-500 border border-gray-100">
                  {plan.cta}
                </div>
              ) : (
                <button
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl border border-gray-100 transform transition-all text-center relative">
            <button 
              onClick={() => setShowModal(false)}
              className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 font-bold px-2 py-1"
            >
              ✕
            </button>
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-2">Upgrade Initiated</h3>
            <p className="text-sm text-gray-500 mb-6 leading-relaxed">
              You selected the <strong className="text-purple-600 font-semibold">{selectedPlan}</strong> plan 
              ({billingCycle === "yearly" ? "billed annually" : "billed monthly"}).
            </p>
            {session ? (
              <div className="text-xs bg-purple-50 text-purple-700 p-3 rounded-xl mb-6">
                Logged in as <strong>{session.user.email}</strong>. Setting up billing panel redirect...
              </div>
            ) : (
              <div className="text-xs bg-amber-50 text-amber-700 p-3 rounded-xl mb-6">
                Please Sign In to finalize your subscription configuration.
              </div>
            )}
            <button
              onClick={() => setShowModal(false)}
              className="w-full py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-xl text-sm font-semibold shadow-sm transition-all"
            >
