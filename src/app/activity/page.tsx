...
export default function ActivityPage() {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-gray-50/50 overflow-hidden">
      {/* Header */}
      <div className="px-6 py-6 border-b border-gray-200 bg-white flex items-center justify-between flex-shrink-0">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Workspace Activity</h1>
          <p className="text-xs text-gray-500 mt-1">Real-time background execution log of AI calls and synced integration events.</p>
        </div>
        <button
          onClick={() => refetch()}
          disabled={isLoading}
          className="flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-4 py-2 text-sm text-gray-700 font-semibold hover:bg-gray-50 active:scale-98 disabled:opacity-50 transition-all shadow-sm cursor-pointer"
        >
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
        {isLoading ? (
          ...
          <div className="bg-red-50 border border-red-200 rounded-2xl p-6 text-red-700 text-sm max-w-2xl mx-auto text-center shadow-sm">
            <h3 className="font-bold text-base mb-1">Failed to fetch activities</h3>
            <p>{error.message}</p>
          </div>
        ) : !events || events.length === 0 ? (
          <div className="max-w-2xl mx-auto mt-8 bg-white border border-gray-200 rounded-3xl p-12 text-center shadow-sm">
                  ...
                  <tr key={event.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <button
                        ...
                        className="bg-white hover:bg-gray-50 border border-gray-200 text-xs font-semibold text-gray-600 px-3 py-1.5 rounded-lg shadow-sm transition-all cursor-pointer"
                      >
                        Inspect
                      </button>
                    </td>
            ...
            {totalPages > 1 && (
              <div className="px-6 py-4 border-t border-gray-200 bg-gray-50/50 flex items-center justify-between">
                <div className="text-xs text-gray-500 font-medium">
                  Showing page {page} of {totalPages} <span className="text-gray-300 mx-1">|</span> {total} total events
                </div>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setPage((p) => Math.max(1, p - 1))}
                    disabled={page === 1}
                    className="px-3 py-1.5 border border-gray-200 rounded-lg text-xs font-semibold text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm transition-all cursor-pointer"
                  >
                    Previous
                  </button>
                  <button
      ...
      {selectedEvent && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[80vh] flex flex-col shadow-2xl border border-gray-100 transform transition-all relative overflow-hidden">
            {/* Modal Header */}
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50">
              <div>
                <h3 className="text-lg font-bold text-gray-900">Event Details</h3>
                <p className="text-[10px] text-gray-400 font-mono mt-0.5">ID: {selectedEvent.id}</p>
              </div>
              <button
            ...
            <div className="flex-1 p-6 overflow-y-auto custom-scrollbar space-y-4">
              <div className="grid grid-cols-2 gap-4 text-xs">
                <div>
                  <span className="text-gray-400 block mb-1">Event Type</span>
                  <span className="font-mono bg-gray-100 px-2 py-1 rounded font-bold text-gray-800">{selectedEvent.eventType}</span>
                </div>
                <div>
                ...
                <div>
                  ...
                  <span className="text-gray-700 font-medium">{new Date(selectedEvent.createdAt).toLocaleString()}</span>
                </div>
                <div>
                  <span className="text-gray-400 block mb-1">Last Updated</span>
                  <span className="text-gray-700 font-medium">{new Date(selectedEvent.updatedAt).toLocaleString()}</span>
                </div>
              </div>

              <div>
                <span className="text-gray-400 text-xs block mb-2">Payload Data (JSON)</span>
                <pre className="bg-gray-900 text-gray-100 p-4 rounded-xl font-mono text-xs overflow-x-auto text-left max-h-[40vh] custom-scrollbar shadow-inner leading-relaxed">
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex justify-end bg-gray-50/50">
              <button
                onClick={() => setSelectedEvent(null)}
                className="bg-gray-900 hover:bg-gray-800 text-white px-5 py-2 rounded-xl text-xs font-semibold shadow-sm transition-all cursor-pointer"
              >
                Close
              </button>
            </div>
